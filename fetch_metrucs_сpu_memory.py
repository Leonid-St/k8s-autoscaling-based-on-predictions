...
while current_start < end_time:
    ...
    if cpu_data["status"] == "success" and memory_data["status"] == "success":
        ...
        cpu_map = {int(value[0]): float(value[1]) for result in cpu_data["data"]["result"] for value in result["values"]}
        memory_map = {int(value[0]): float(value[1]) for result in memory_data["data"]["result"] for value in result["values"]}
...
dump_data_to_file(node_data_map, 'node_data_cpu_memory_130592_623a030d0392.json')
